  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Total Deposit", value: totalDeposit, cls: "" },
          { label: "Total Withdrawal", value: totalWithdraw, cls: "text-down" },
          { label: "Net Investment", value: totalDeposit - totalWithdraw, cls: totalDeposit - totalWithdraw >= 0 ? "" : "text-down" },
        ].map((item) => (
      ...
      {cashFlows.length === 0 ? (
        <p className="py-8 text-center text-muted-foreground text-sm">No cash flow records</p>
      ) : (
              ...
              {cashFlows.map((c) => (
                <tr key={c.id} className="border-b border-border/50 hover:bg-accent/30">
                  ...
                  <td className="py-3 pl-3">
                    <button
                      onClick={() => { if (confirm("Delete?")) deleteCashFlow(c.id); }}
